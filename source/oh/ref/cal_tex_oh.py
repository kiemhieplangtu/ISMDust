...
def cal_tex(data):
	...
	for src in s26src:
		if(src in src_list):
			...
			# Plot #
			fig    = cplot()
			trace1 = fig.lines(velo,10000.*tau,label='Tau*1e5',
					prop=dict(color='b',
						      linewidth=3,
						      linestyle='solid',
						      marker='o',
			...
			layout = dict(title  = src+' - OH(1667): Tex_mean = '+str(round(tex_mean,2))+' within FWHM',
						  title_fontsize=30,
			              grid   = True,
			              legend = dict(loc='upper left', fontsize=18),
			              xaxis  = dict(label='VLSR(km/s)',tick_size=18,fontsize=35,xlim=[xmin,xmax]),
			              yaxis  = dict(label='Tex(K)',tick_size=18,fontsize=35,ylim=[-200.,200.]),
