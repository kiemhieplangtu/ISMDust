...
sc94   = md.read_nhi_93src(fname = '../result/nhi_thin_cnm_wnm_94src_scaled.txt')
sc     = sc94['src']
xl     = sc94['l']
xb     = sc94['b']
hi     = sc94['nhi']
...
info  = md.read_info_no_co('../../co12/result/26src_no_CO_scaled.txt')
sc26  = info['src']
k     = 0
for i in range(len(sc)):
	if ( (hi[i] < 4.0) and (sc[i] not in sc26) ):
		k = k + 1
