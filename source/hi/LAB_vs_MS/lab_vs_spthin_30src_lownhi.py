...
def cal_nh_from_dust(map_file, info, lowhi):
	...
	# plt.text(0.05, 5.0, 'Far from 1.0 are l-o-s with very low N(HI) (<3.0e20)', color='blue', fontsize=20)
	...
	# plt.plot([0,3.5],[0,3.5], 'k--', label='$N^*_{H} = N^{LAB}_{HI}$')

	plt.title('$N^*_{HI}$ and $N^{LAB}_{HI}$ along 30 SPONGE lines-of-sight', fontsize=30)
	...
	plt.text(0.1, 2.5, '$f = ['+str(m)+'\pm'+str(ea) +']\cdot (N^*_{HI}/10^{20})$', color='blue', fontsize=20)
	...
	plt.fill_between(xfit, mu - sig, mu + sig, color='0.5', alpha=0.5)
	# plt.plot([0,60],[0,], 'k--', label='$x=y$')

	plt.title('$N^*_{HI}$ and $N^{LAB}_{HI}$ along low N(HI) SPONGE lines-of-sight', fontsize=30)
	...
	plt.text(1.2, 2.2, '$y = ['+str(m)+'\pm'+str(ea) +']\cdot x$', color='blue', fontsize=20)
	plt.legend(loc='upper left', fontsize=18)
	# plt.savefig("test.png",bbox_inches='tight')
	# for i in range(len(src)):
	# 	plt.annotate('('+str(src[i])+', '+ str(xl[i])+', '+str(xb[i])+')', xy=(xdata[i], ydata[i]), xycoords='data',
...
info     = read_info_sponge_30src(fname = '../../oh/sponge/sub_data/30src_claire.txt')
