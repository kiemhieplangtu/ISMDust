...
def cal_tex(data, inf408):
	...
	cont_em    = tbg
	tbaseline  = round(bg_off1665,3)+0.1
	...
	bsl = []
	er  = []
	# for x in np.arange(78.12/2.,78.14/2.,0.001):
	for x in np.arange(39.167,39.1752,0.001):  #=> 39.171
		tfita, sigma, \
		zrolvfite1, taufite1, v0fite1, widfite1, texfite1, \
		zrolv_ere1, tau_ere1, v0_ere1, wid_ere1, tex_ere1, \
		cont_eme1,\
	...
	plt.plot(xd,tdfit)
	plt.plot(xd, tb_tot,'r-')
	plt.title('1667 Emission - 3C409')
	plt.xlim(xmin, xmax)
	plt.show()

	plt.plot(bsl,er, 'r-', lw=3)
	plt.title('Residuals vs baseline')
	plt.show()

	sys.exit()

	## After check Chi2 vs baseline ##
	tbaseline  = 39.17 #78.13/2. #@78.133
	tdfit      = tde - tbaseline + tbg
