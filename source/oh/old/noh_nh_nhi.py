...
fmt  = ['i','s','f','f','f','f','f','f']
data = restore('sub_data/nh_nhi_uncert_78src.txt', 3, cols, fmt)
dat  = data.read()
...
for sc in noh2:
	if((sc in src) and (noh2[sc] > 0.)):
		...
		print("{}   {}\t{}\t{}\t{}\t{}\t{}\t{}" \
			.format(n, sc, noh2[sc]['noh'], np.sqrt(noh2[sc]['er2']), nhi[src.index(sc)],nhi_er[src.index(sc)], nh[src.index(sc)],nh_er[src.index(sc)]) )
		n = n + 1 
