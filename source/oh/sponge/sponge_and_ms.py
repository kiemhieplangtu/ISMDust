...
## Read 78 MS sources #
 #
 # params string fname Filename
 # return dict info of 78 MS sources
 # 
 # version 10/2016
 # Author Van Hiep ##
def read_78ms_src(fname = '../sub_data/nh_nhi_uncert_78src.txt'):
	cols = ['idx','src','noh','noh_er','nhi','nhi_er','nh','nh_er']
	fmt  = ['i','s',  'f','f',  'f','f',  'f','f']
	data = restore(fname, 3, cols, fmt)
	return data.read()
...
## get common src and different src between Claire's results and 78-MS src ##
 # params list src78 78 MS sources
 # params list src30 30-src from Claire's results
 #
 # return list common Common src 
 #        list diff   different src 
 ...
 # version 10/2016
 # Author Van Hiep ##
def get_common_diff_mssrc_with_claire(src78,src30):
	common = []
	diff   = []
	for sc in src30:
		if(sc in src78):
...
## get common src and different src between Sponge and 78-MS src ##
 # params list src78 78 MS sources
 # params list src51 51 src from Sponge
 #
 # return list common Common src 
 #        list diff   different src 
 #
 # version 10/2016
 # Author Van Hiep ##
def get_common_diff_mssrc_with_sponge(src78,src51):
	common = []
	diff   = []
	for sc in src51:
		if(sc in src78):
...
## READ 78 MS Sources ##
ms78src = read_78ms_src('../sub_data/nh_nhi_uncert_78src.txt')
src78   = ms78src['src']
common3,diff3 = get_common_diff_mssrc_with_claire(src78,src30)

print 'Common src'
for i in range(len(common3)):
	print i, common3[i], get_idx(common3[i],sc_names)

print 'Diff src'
for i in range(len(diff3)):
	print i, diff3[i], get_idx(diff3[i],sc_names)

## READ Sponge & MS Sources ##
common3,diff3 = get_common_diff_mssrc_with_sponge(src78,src51)
print 'Common src'
for i in range(len(common3)):
	print i, common3[i], get_idx(common3[i],sc_names)
