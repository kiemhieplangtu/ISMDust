...
def cal_tex(data, inf408, bd=1):
	...
	bg408    = read_tbg408_healpy('../result/bg408_to_compare.txt')
 	...
 	for src in src_list:
		...
		# plt.fill_between(xfit, Tfita/contfit-sigma/contfit, Tfita/contfit+sigma/contfit, color='0.5', alpha=0.5)
		# plt.title(src, fontsize=30)
		plt.ylabel(r'$e^{-\tau}$', fontsize=20)
		plt.xlim([xmin, xmax])
		ymin = min(Tfita/contfit)
		ymax = max(Tfita/contfit)
		plt.ylim([0.9913, 1.002])
		...
		yy1 = 0.996
		plt.text(-2.1, yy1, '$\mathrm{P0428}$+${20}$', fontsize=14, fontweight='bold')
		plt.text(-2.1, yy1-0.0005, '$\mathrm{(L,B)\ \ =(176.81,\ \ -18.56)}$', fontsize=14, fontweight='bold')

		plt.text(-2.1, yy1-0.0015, '$\mathrm{OH\ (1665)}$', fontsize=14, fontweight='bold')
		plt.text(-2.1, yy1-0.002, r'$\tau\ \ \ \ \ \ \ \ \ =[0.0015,\ \ 0.0076]$', fontsize=14, fontweight='bold')
		plt.text(-2.1, yy1-0.0025, '$\mathrm{VLSR}\ \ =[3.60,\ \ 10.70]$', fontsize=14, fontweight='bold')
		plt.text(-2.1, yy1-0.003, '$\mathrm{FWHM}=[1.06,\ \ 1.10]$', fontsize=14, fontweight='bold')
		plt.text(-2.1, yy1-0.0035, '$\mathrm{T_{ex}}\ \ \ \ \ \ =[8.424,\ \ 8.420]$', fontsize=14, fontweight='bold')
		...
		plt.xlim([xmin, xmax])
		plt.ylim([-0.05, 0.06])
		plt.yticks([-0.04, -0.02, 0.0, 0.02, 0.04])
		plt.gca().spines["top"].set_visible(False)
		plt.axhline(y=0.06, xmin=-100, xmax=100, c='k', ls='--', linewidth=4) ## This line is important, don't forget
