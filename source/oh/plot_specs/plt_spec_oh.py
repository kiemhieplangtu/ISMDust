...
def get_spec_data(src, srcs, data):
	...
	return [x1,t_on1,t_off1, bg_on1, bg_off1, xmin, xmax]
...
def plot_1665(data):
	...
	for src in srcs:
		...
		v,ton,toff,bgon,bgoff,xmin,xmax = get_spec_data(src, srcs, data)
		...
		layout = dict(title  = 'Absorption line: ' + src,
		              ...
		              xaxis  = dict(label='$V_{lsr} (km/s)$',tick_size=18,fontsize=35,xlim=[-20.,20.]),
		...
		fig.iplot(dat,layout,fullscreen=True)
