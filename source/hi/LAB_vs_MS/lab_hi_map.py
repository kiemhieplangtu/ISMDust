...
###================= MAIN ========================###
# The data were resampled on NSIDE=512 HEALPix grid using nearest neighbour pixel.
# The NSIDE=512 with approx 0.1 sq deg pixels is probably oversampling the original data, 
# which is stored in approximately 0.25 sq deg pixels. The HEALPix map is a two-channel map: 
# the first channel (the TEMPERATURE field) contains the HI brightness temperature in K averaged 
# averaged over velocity channels, while the second channel (the SIMULATION field) is the number 
# of original LAB velocity channels that went into that particular HEALPix pixel. 
# The velocity channel spacing is 1.030571969 km s-1. With exception of thin velocity slices, 
# the sky coverage is pretty uniform.
deg2rad  = np.pi/180.
dv       = 1.030571969
cst      = 1.8224e18
