...
## Get NHI_thin from LAB HI data #
 ...
 # 
 # Author Van Hiep ##	
def cal_nhi_from_LAB(map_file, info):
	src     = info['src']  ## the sources
	nhi     = info['thin']
	nhi_er  = info['thin_er']
	...
	for i in range(len(src)):
		...
		lhier.append(err)
		lwnhi.append(nhi[i])
		lwhier.append(nhi_er[i])

	return lhi, lhier, lwnhi, lwhier

## Get N(HI) values and err_tau353 values #
 #
 # params str map_file File of maps
 # params str LAB_file File of HI LAB maps
 # params dict info   Information of sources
 ...
 # Author Van Hiep
 ##
def cal_nh_from_dust(map_file, LAB_file, info, lownhi):
	src   = info['src']  ## 78 src
	nhi   = info['thin']
	nhier = info['thin_er']
	xl    = info['l']
	...
	lhi, lhier, nhi, nhier = cal_nhi_from_LAB(LAB_file, info)       ## 23 src with low_nhi
	...
	plt.title('$N^*_{HI}$ and $N^{LAB}_{HI}$ along 78 lines-of-sight', fontsize=30)
	...
	plt.title('$N^*_{HI}$/$N^{LAB}_{HI}$ along 78 Millennium Survey lines-of-sight', fontsize=22)
	...
	plt.title('$N^*_{HI}$ and $N^{LAB}_{HI}$ along 78 Millennium Survey lines-of-sight', fontsize=30)
	plt.ylabel('$N^{thin}_{HI} [10^{20}$ cm$^{-2}]$, from MS data', fontsize=35)
	plt.xlabel('$N^{thin}_{HI} [10^{20}$ cm$^{-2}]$, LAB data', fontsize=35)
	# plt.xlim(0.0, 2.0)
	# plt.ylim(-1.0, 6.0)
	plt.grid(True)
	...
	plt.text(0.2, 30.0, '$f = ['+str(m)+'\pm'+str(ea) +']\cdot (N^*_{HI}/10^{20})$', color='blue', fontsize=20)
	plt.legend(loc='upper left', fontsize=18)
	# plt.savefig("test.png",bbox_inches='tight')
	# for i in range(len(src)):
...
cal_nh_from_dust(map_file, LAB_file, info, lowhi)
