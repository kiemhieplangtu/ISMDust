...
def cal_tex(data):
	...
	for src in src_list:
		...
		noh     = 2.21841851219*tex_mean*stau         # x10^14
		noh_fit = 2.21841851219*tex_mean*stau_fit[0]  # x10^14
		# print '======'
		...
		# layout = dict(title  = src+' - Tau of OH(1667): Tex_mean = '+str(round(tex_mean,2))+' within FWHM \n N(OH): '+str(round(noh,3))+'x$10^{14}$ cm$^{-2}$)'+', N(OH): '+str(round(noh_fit,3))+'x$10^{14}$ cm$^{-2}$)',
		# 			  title_fontsize=30,
		#               grid   = True,
