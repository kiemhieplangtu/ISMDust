...
def plot_haslam_408mhz_patch():
	...
	for i in range(len(src)):
		...
		xf = 0.1
		hp.cartview(tb408/1000., title=src[i], coord='G', unit='K',
				norm='hist', xsize=800, lonra=[ll-offset-xf*offset,ll+offset+xf*offset], latra=[sb-offset-xf*offset,sb+offset+xf*offset],
		...
		tbgx  = 2.8+val*(408./1665.402)**2.8

		if(i<18):
			print '{0}\t\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}'\
			.format(src[i], gl[i], gb[i], int(il[i]), int(ib[i]), tbg[i], int(lid[i]), int(bid[i]), bg1[i], val, tbgx )
		else:
			print '{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}'\
			.format(src[i], gl[i], gb[i], int(il[i]), ib[i], tbg[i], int(lid[i]), int(bid[i]), bg1[i], val, tbgx )
