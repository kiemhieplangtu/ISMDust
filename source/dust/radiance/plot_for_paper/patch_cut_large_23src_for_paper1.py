...
ohsrc, xl, xb     = read_23src_with_OH(fname = '../../../oh/result/23src_with_oh_1.dat')
...
for i in range(4):
	...
	if(np.all(zd.min() != zd.max())):
		...
		xmax = zd.max()
		xmin = zd.min()
		axarr[2,i].contour(xd, yd, zd, np.arange(xmin, xmax, (xmax-xmin)/5. ), colors='k', linestyles='solid')
...
fig.tight_layout()
fig.savefig('src_examples_apd1.eps', format='eps', bbox_inches='tight', pad_inches=0.05, dpi=100)
fig.show()