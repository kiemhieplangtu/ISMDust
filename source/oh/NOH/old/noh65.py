...
def peak_info(source,fname='../data/gauss_1665_peaks.txt'):
	...
	for i in range(0,len(src)):
		if info['src'][i] not in ret.keys():
			...
			ret[src[i]]['base_range'] = [bmin[i],bmax[i]]
			ret[src[i]]['guessp']     += [tau[i],v0[i],wid[i]]
	print source, ret[source]['guessp']
	...
	bg408    = read_tbg408_healpy()
 	...
 	for src in src_list:
		...
		if(xplot):
			...
			plt.plot(x,etau, 'b.-', label='data - Absorption line', ms=10)
			...
			## Emission line ##
			plt.plot(x,tde[xmin_id:xmax_id], 'b.-', label='data - Emission line', ms=10)
			for i in range(2,len(abp),3):
				plt.axvline(abp[i]-abp[i+1]/2.,ymin=-10., ymax=1000.,linewidth=3,color='k')
				plt.axvline(abp[i]+abp[i+1]/2.,ymin=-10., ymax=1000.,linewidth=3,color='k')
			plt.title(src,fontsize=35)
		...
		for i in range(1,len(abp),3):
			...
			taufit_sig.append(abper[i])
			v0fit_sig.append(abper[i+1])
			widfit_sig.append(abper[i+2])
		...
		for k in range(npeak):
			...
			noh_peak[k]    = 4.26413*tex_peak[k]*tau_fit[k]*wid_fit[k]
			noh_sig[k]     = cal_noh65_uncertainty(tau_fit[k], wid_fit[k], tex_peak[k], taufit_sig[k], widfit_sig[k], texsig_peak[k])
			# else:
			# 	tex_peak[k]    = 0.
			# 	noh_peak[k]    = 0.
			# 	texsig_peak[k] = 0.
		...
		## PLOT On-Source LINE: t_on  = tde + td##
		if(xplot):
			plt.plot(xe,ton,'b.-', label='On-source spectrum, $T_{on} = T_{abs} + T_{off}$', ms=10)
