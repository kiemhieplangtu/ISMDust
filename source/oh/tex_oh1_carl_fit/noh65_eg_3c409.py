...
def cal_tex(data, inf408, bd=1):
	...
	bg408    = read_tbg408_healpy('../result/bg408_to_compare.txt')
 	...
 	for src in src_list:
		...
		# plt.plot(v,Ta)
		# plt.plot(xfit, Tfita, 'r-', lw=2)
		# plt.title(data.la[n].srcname+' - 1665 Absortion line Fit')
		# plt.xlim([xmin, xmax])
		# ymin = min(Tfita)
		# ymax = max(Tfita)
		# plt.ylim([ymin-0.002*ymin, ymax+0.002*ymax])
		# plt.grid()
		# plt.show()
		...
		# tb_tot = fit.tb_exp(v, \
		# 		zrolvfite, taufite, v0fite, widfite, texfite, cont_eme)
		...
		# plt.plot(v,Tefit, 'k-', lw=1, label='data')
		# # plt.plot(v, tb_tot,'m-', lw=2, label='Tb_tot')
		# plt.plot(xfit, Tfite,'r-', lw=3, label='$T_{exp}$ fit')
		# plt.title(data.la[n].srcname+' - 1665 Expected Profile Final fit')
		# plt.xlim([xmin, xmax])
		# ymin = min(Tfite)
		# ymax = max(Tfite)
		# plt.ylim([ymin-0.008*ymin, ymax+0.008*ymax])
		# plt.grid()
		# plt.show()
		...
		frame1=fig1.add_axes((.1,.5,.8,.46))
		plt.plot(v,Ta/contfit, 'k-', linewidth=1, label='')
		plt.plot(xfit, Tfita/contfit, 'k-', linewidth=2, label='')
		# plt.title(src, fontsize=30)
		plt.ylabel(r'$e^{-\tau}$', fontsize=20)
		plt.xlim([xmin, xmax])
		ymin = min(Tfita/contfit)
		ymax = max(Tfita/contfit)
		plt.ylim([0.9755, 1.002])
		# plt.ylim([ymin-0.002*ymin, ymax+0.002*ymax])
		plt.tick_params(axis='y', labelsize=15)
		plt.tick_params(axis='x', labelsize=18, labelbottom='off')
		plt.tick_params(axis='y',which='both', width=1)
		plt.tick_params(axis='y',which='major', length=5)
		plt.tick_params(axis='x',which='both', width=1)
		plt.tick_params(axis='x',which='major', length=5, bottom='off')
		plt.gca().spines["bottom"].set_visible(False)

		yy1 = 0.986
		plt.text(7.9, yy1, '$\mathrm{3C409}$', fontsize=14, fontweight='bold')
		plt.text(7.9, yy1-0.001, '$\mathrm{(L,B)\ \ =(63.40,\ \ -6.12)}$', fontsize=14, fontweight='bold')

		plt.text(7.9, yy1-0.003, '$\mathrm{OH\ (1665)}$', fontsize=14, fontweight='bold')
		plt.text(7.9, yy1-0.004, r'$\tau\ \ \ \ \ \ \ \ \ =[0.0062,\ \ 0.0199]$', fontsize=14, fontweight='bold')
		plt.text(7.9, yy1-0.005, '$\mathrm{VLSR}\ \ =[14.63,\ \ 15.40]$', fontsize=14, fontweight='bold')
		plt.text(7.9, yy1-0.006, '$\mathrm{FWHM}=[1.86,\ \ 0.89]$', fontsize=14, fontweight='bold')
		plt.text(7.9, yy1-0.0072, '$\mathrm{T_{ex}}\ \ \ \ \ \ =[7.96,\ \ 2.35]$', fontsize=14, fontweight='bold')

		# plt.legend(loc='upper left', fontsize=18)
		plt.grid(False)

		frame3=fig1.add_axes((.1,.4,.8,.1))
		plt.plot(xfit, resida/contfit, 'k-', linewidth=1, label='')
		plt.ylabel(r'$\rm{Residuals}$', fontsize=18)
		plt.tick_params(axis='y', labelsize=13, pad=5)
		plt.tick_params(axis='x', labelsize=18, labelbottom='off')
		plt.tick_params(which='both', width=1)
		plt.tick_params(which='major', length=5, top='off')
		plt.xlim([xmin, xmax])
		plt.ylim([-0.0025, 0.0028])
		plt.yticks([-0.002, -0.001, 0.0, 0.001, 0.002])
		plt.gca().spines["top"].set_visible(False)
		plt.axhline(y=0.0028, xmin=-100, xmax=100, c='k', ls='--', linewidth=4)

		## --------------- ##
		frame2=fig1.add_axes((.1,.2,.8,.194))
		plt.plot(v,Tefit-tbg, 'k-', lw=1, label='')
		plt.plot(xfit, Tfite-tbg,'k-', lw=2, label='')
		# plt.plot(xfit, 4.45053 + reside-tbg, 'k-', linewidth=1, label='')
		# plt.fill_between(xfit, Tfite-sigmae, Tfite+sigmae, color='0.5', alpha=0.5)
		frame2.set_ylabel(r'$\rm{T}_{\rm{exp}}\ (K)$',fontsize=20)
		...
		ymin = min(Tfite)
		ymax = max(Tfite)
		# plt.ylim([ymin-0.01*ymin, ymax+0.01*ymax])
		plt.ylim([-0.13, 0.13])
		# plt.legend(loc='upper left', fontsize=18)
		plt.grid(False)
		...
		frame4=fig1.add_axes((.1,.1,.8,.1))
		plt.plot(xfit, reside, 'k-', linewidth=1, label='')
		plt.ylabel(r'$\rm{Residuals}$', fontsize=18)
		plt.xlabel('$\mathrm{VLSR\ (km/s)}$', fontsize=20)
		...
		plt.gca().spines["top"].set_visible(False)
		plt.axhline(y=0.14, xmin=-100, xmax=100, c='k', ls='--', linewidth=4) ## This line is important, don't forget
		
		# plt.tight_layout()
		plt.savefig('abs_em_oh1_'+src+'.eps', format='eps', dpi=600)
		plt.show()
		...
		for k in range(npeak):
			...
			print '{}  {}\t{}\t{:04.8f}\t{:04.6f}\t{:04.6f}\t{:04.6f}\t{:04.6f}\t{}\t{:04.6f}\t{}'\
						.format(n, src, str(tauk)+'/'+str(ntauk), tauerk, v0k, v0erk, widk, widerk, str(texk)+'/'+ str(ntexk), tex_erk, str(texk)+'/'+ str(ntexk) )
