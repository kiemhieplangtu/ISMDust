...
def nh_factor_vs_nhi():
	data    = read_nh_nhi(fname = 'result/nh_fromCO_16src_f2.txt')
	src1    = data['src']
	...
	layout = dict(title  = 'Correlation between Factor $f = N_{H}$/$N_{HI}$ and HI column density $N_{HI}$ \nalong 42 lines-of-sight',
	              ...
	              text   = [dict(loc=[0.2,0.4],text='a = '+str(m)+'$\pm$'+str(ea) +',  b = '+str(b)+'$\pm$'+str(eb),color='blue',fontsize=17),
	              			dict(loc=[1.8,5.4],text='$X = [2.0\pm0.6]10^{20} [H_{2}cm^{-2}/(K km/s)]$',color='blue',fontsize=17),
	              			dict(loc=[1.8,5.2],text='N(H) = N(HI) + 2Xco*Wco',color='blue',fontsize=19),
	              			dict(loc=[1.8,5.0],text='$tau_{353} = [8.4\pm3.0]10^{-27}N_{H}$, and no CO',color='red',fontsize=19),
	              		   ],
	             )
