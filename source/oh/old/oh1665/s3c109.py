...
def main(data, inf408):
	...
	for src in src_list:
		...
		tbaseline  = 52.215  # baseline of 1665 Absorption line
		...
		plt.plot(xdat, ta_fit)
		plt.plot(xdat, ta_dat)
		plt.show()
		...
		for i in range(len(peakindx)):
			print 'x0 peak:', x[peakindx[i]]


		vpeaks = [9.22,9.91,9.92, 10.96]
		
		plt.plot(xdat, ts_fit)
		plt.plot(xdat, ts_dat)
		plt.ylim(-1.,30.)
		for i in range(len(vpeaks)):
			plt.axvline(vpeaks[i],0.,1)
		plt.show()
