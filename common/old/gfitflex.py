...
class gfit:
	...
	def fit(self,look, xdataa, tdataa, xindxrange, \
		...
		while (redoit == 1):
			...
			## FIRST DEFINE SFULL...
			sum_of_gaussians = self.gcurv(xdata, zro1, hgt1, cen1, wid1/dp600)
	...
	##  Fit the Expected Emission Profile ##
	 ...
	 # version 01/2017 
	 # author Nguyen Van Hiep ##
	def efit(self,look, xdataa, tdataa, xindxrange, \
			...
			zrownmyn, hgtwnmyn, cenwnmyn, widwnmyn, fwnmyn, nloopmax=50, halfasseduse=0.5):
		...
		# HALFASSED IS THE MULTIPLIER FOR THE CORRECTIONS IN NONLI!=AR REGIME.
		halfassed = np.float64(halfasseduse)
		...
		if(type(hgtwnmyn) is int): ## when ONLY 1 single component, so hgtwnmyn=1, need to be a list hgtwnmyn=[1]
			hgtwnmyn = [hgtwnmyn]
			cenwnmyn = [cenwnmyn]
			widwnmyn = [widwnmyn]
			fwnmyn   = [fwnmyn]
		...
		# for j in range(1):
			...
			for ng in range(ngaussians_cnm):
				...
				xdel          = np.float64(0.00025) ## Different
			...
			for ng in range(ngaussians_wnm):
				## EVALUATE HGT DERIVATIVE:
				# xdel = np.float64(0.0000025)
				xdel = np.float64(0.000025) # changed
			...
			# CHECK TO SEE IF WIDTH IS TOO BIG..but ignore if these params are fixed.
			if ((max(np.array(widcnmyn)*widcnm1) > dfstop)  or \
				(min(np.array(widcnmyn)*widcnm1) < 0.)     or \
				(max(np.array(widwnmyn)*widwnm1) > dfstop) or \
				(min(np.array(widwnmyn)*widwnm1) < 0.) ):
			    problem = -1
			    break
