...
def cal_tex(data, inf408):
	...
	for src in src_list:
		...
		print popt
		print ''
		print 'n,src,tbg,bg_off,trx,bg_on'
		print 'Mean Tex'
		print n,src,tbg,bg_off,trx,bg_on
		print tex_mean
		print '====== END ======'			
		...
		# Plot #
		fig    = cplot()
		trace1 = fig.lines(x,t_off,label='T_off',
				prop=dict(color='b',
					      linewidth=1,
					      linestyle='solid',
					      marker='o',
		...
		layout = dict(title  = src+' - OH(1665): Tex_mean = '+str(round(tex_mean,2))+' within FWHM',
		              ...
		              xaxis  = dict(label='VLSR(km/s)',tick_size=18,fontsize=35,xlim=[-40,40]),
		              yaxis  = dict(label='Toff(K)',tick_size=18,fontsize=35),
