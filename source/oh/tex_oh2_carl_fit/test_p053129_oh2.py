...
def cal_tex(data, inf408):
	...
	## Background ##
	tbg1667 = 2.8+get_tb_408(ell[n], bee[n], inf408.tb_408)*(408./1667.402)**2.8 # Tbg from 408MHz
	# tbg1665 = 3.70
	...
	tc1667     = baseline_from_linear_fit(x, ab_avg2[n], avmin1[n], avmax1[n], avmin2[n], avmax2[n])
	bg_off1667 = baseline_from_linear_fit(x, em_avg2[n], evmin1[n], evmax1[n], evmin2[n], evmax2[n])
	# tc1665 = 59.15/2.
	# bg_off1665 = 70.9/2
	print '***********************'
	print n, ' ', data.la[n].srcname, data.la[n].ell, data.la[n].bee
	print 'Tc: ',tc1667
	print 'Emission baseline: ',bg_off1667
	print 'Tbg_from_408: ',tbg1667	
	print '-------- Results ---------'
	...
	plt.title(data.la[n].srcname+' - 1667 Absortion line Fit')
	plt.xlim(-5.0, 11.)
	plt.show()
	...
	# bsl = []
	# er  = []
	# for x in np.arange(37.387,37.389,0.0001): ## 37.390 NOT WORK
	# 	tfita, sigma, \
	...
	d  = 2.36634*0.00089071*4.00867*1.0528
	er = d*np.sqrt((0.00024/0.00089)**2 + (0.346/1.053)**2 + (7.10/4.01)**2)
	print '    ', d, 'x10^14'
	print '+/- ', er, 'x10^14'
